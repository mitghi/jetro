...
pub enum Val {
    ...
    StrSlice(crate::data::tape::StrRef),
    ...
    StrSliceVec(Arc<Vec<crate::data::tape::StrRef>>),
...
/// [`Val::from_value_with`] / [`Val::from_tape_data_with`] instead, which
/// thread an engine-owned [`crate::data::intern::KeyCache`].
#[inline]
...
mod valref_tests {
    ...
    fn from_tape_data_uses_borrowed_string_slices_and_preserves_json() {
        ...
        let tape = crate::data::tape::TapeData::parse(js.clone()).unwrap();
    ...
    fn from_tape_data_promotes_float_arrays_and_indexes_str_slice_vec() {
        ...
        let tape = crate::data::tape::TapeData::parse(js).unwrap();
        let val = Val::from_tape_data(&tape);
        let obj = val.as_object().unwrap();
