...
impl JetroEngine {
    ...
    /// Treat an NDJSON input as one virtual array, evaluate `query` once, and
    ...
    where
        ...
        W: std::io::Write,
    {
        io::run_ndjson_stream(self, reader, query, writer)
    }

    /// Like [`JetroEngine::run_ndjson_stream`] with explicit NDJSON reader options.
    ...
    where
        ...
        W: std::io::Write,
    {
        io::run_ndjson_stream_with_options(self, reader, query, writer, options)
    }

    /// Open an NDJSON file as one virtual array, evaluate `query` once, and
    /// write the single JSON result.
    ...
    where
        ...
        W: std::io::Write,
    {
        io::run_ndjson_stream_file(self, path, query, writer)
    }

    /// Like [`JetroEngine::run_ndjson_stream_file`] with explicit NDJSON reader options.
    ...
    /// Treat an NDJSON input as one virtual array and evaluate `query` once.
    ...
    where
        R: std::io::BufRead,
    {
        io::collect_ndjson_stream(self, reader, query)
    }

    /// Like [`JetroEngine::collect_ndjson_stream`] with explicit NDJSON reader options.
    ...
    where
        R: std::io::BufRead,
    {
        io::collect_ndjson_stream_with_options(self, reader, query, options)
    }

    /// Open an NDJSON file as one virtual array and evaluate `query` once.
    ...
    where
        P: AsRef<std::path::Path>,
    {
        io::collect_ndjson_stream_file(self, path, query)
    }

    /// Like [`JetroEngine::collect_ndjson_stream_file`] with explicit NDJSON reader options.
