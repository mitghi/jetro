...
pub struct Jetro {
    ...
    /// [`Jetro::from_bytes`] / [`Jetro::from_slice`]. Enables tape
    /// materialisation and VM descendant scan fallbacks without
    /// requiring callers to keep their input buffer alive.
...
fn try_tape_array_map_aggregate(
    ...
    if map_idx == 0 {
        return None;
    } // need at least one field to land on an array
...
fn classify_pred<'a>(e: &'a crate::ast::Expr) -> Option<crate::strref::TapePred<'a>> {
    ...
    use crate::strref::{TapeCmp, TapePred};
    match e {
        ...
        Expr::BinOp(lhs, op, rhs) => {
            ...
            } else if let (Some(lit), Some(f)) =
                (classify_literal(lhs), classify_implicit_field(rhs))
            {
...
fn try_tape_array_filter_count(
    ...
    if tail.len() == 1 {
        let (ints, floats, is_float) = crate::strref::tape_array_filter_pred_map_collect_numeric(
...
fn try_tape_descend_aggregate(
    ...
    if steps.len() == 1 {
        ...
        let (ints, floats, is_float) = crate::strref::tape_descend_collect_numeric(tape, key)?;
        let out = if is_float {
            Val::FloatVec(Arc::new(floats))
        } else {
            Val::IntVec(Arc::new(ints))
...
impl Jetro {
    ...
    pub fn from_simd(bytes: Vec<u8>) -> std::result::Result<Self, String> {
        // Validate with the same tape parser used by lazy execution.
        // Keep the successfully parsed tape so the first tape-friendly
        // query can run without reparsing, while still preserving the
        // historical contract that invalid JSON returns an error here.
        let raw: Arc<[u8]> = Arc::from(bytes.clone().into_boxed_slice());
        let tape = crate::strref::TapeData::parse(bytes)
            .map_err(|e| format!("simd-json tape parse failed: {e}"))?;

        // Non-tape-friendly queries materialise Val lazily from the
        // retained tape on first structural access.
        Ok(Self {
            document: Value::Null,
            root_val: OnceCell::new(),
            objvec_cache: Default::default(),
    ...
    pub fn collect<S: AsRef<str>>(&self, expr: S) -> std::result::Result<Value, EvalError> {
        ...
        if let Some(p) = lowered {
            ...
            return p
                .run_with(
                    &self.root_val(),
                    Some(self as &dyn pipeline::ObjVecPromoter),
                )
                .map(|v| v.into());
        }
        ...
        with_vm(|cell| match (cell.try_borrow_mut(), &self.raw_bytes) {
            ...
            (Err(_), Some(bytes)) => {
                VM::new().run_str_with_raw(expr, &self.document, Arc::clone(bytes))
            }
        ...
        W: std::io::Write,
    {
        ...
        let bytes = val.to_json_vec();
        w.write_all(&bytes)
            .map_err(|e| EvalError(format!("write: {}", e)))?;
        Ok(())
    }
    ...
    pub fn collect_val<S: AsRef<str>>(&self, expr: S) -> std::result::Result<JetroVal, EvalError> {
        ...
        self.collect_val_slow(expr, None)
    }

    /// Slow-path tail of `collect_val` (tape-descend / Pipeline::run
    /// / VM fallback). `prelowered` lets callers that already parsed an
    /// expression reuse the lowered Pipeline.
    fn collect_val_slow(
        &self,
        expr: &str,
        ...
        let lowered: Option<pipeline::Pipeline> = match prelowered {
            Some(p) => Some(p),
            None => parser::parse(expr)
                .ok()
                .as_ref()
                .and_then(pipeline::Pipeline::lower),
        };

        // Tape descend-aggregate fast path for `$..k.<aggregate>` shapes.
        #[cfg(feature = "simd-json")]
        if let Some(tape) = self.lazy_tape() {
            let val_built = self.root_val.get().is_some();
            if let Some(out) = try_tape_descend_aggregate(expr, tape, val_built) {
...
impl JetroIter {
    ...
    fn from_expr(j: &Jetro, ast: &ast::Expr) -> std::result::Result<Self, Error> {
        ...
        Ok(JetroIter {
            inner: JetroIterInner::Eager(items.into_iter()),
        })
    }
}
