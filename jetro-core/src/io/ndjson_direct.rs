...
use crate::builtins::{BuiltinLogicalShape, BuiltinViewObjectProjection};
...
fn direct_tape_predicate_from_kernel(
    ...
    match kernel {
        crate::exec::pipeline::BodyKernel::CmpLit { lhs, op, lit } if op.is_scalar_comparison() => {
            if let Some(steps) = kernel_to_physical_path(lhs) {
                return Some(NdjsonDirectPredicate::Binary {
                    lhs: Box::new(NdjsonDirectPredicate::Path(steps)),
                    op: *op,
                    rhs: Box::new(NdjsonDirectPredicate::Literal(lit.clone())),
                });
            }
            if let Some((source_steps, element, suffix_steps)) =
                direct_array_element_path_from_kernel(lhs)
            {
                return Some(NdjsonDirectPredicate::ArrayElementCmpLit {
                    source_steps,
                    element,
                    suffix_steps,
                    op: *op,
            ...
            {
                return Some(NdjsonDirectPredicate::ArrayElementViewScalarCmpLit {
                    source_steps,
                    element,
                    suffix_steps,
                    call,
        ...
        crate::exec::pipeline::BodyKernel::BuiltinCall { .. } => {
            if let Some((source_steps, element, suffix_steps, call)) =
                direct_array_element_scalar_call_from_kernel(kernel)
            {
                return Some(NdjsonDirectPredicate::ArrayElementViewScalarCall {
                    source_steps,
                    element,
                    suffix_steps,
                    call,
            ...
            Some(NdjsonDirectPredicate::ViewScalarCall { steps, call })
        }
        crate::exec::pipeline::BodyKernel::ArraySelect { .. }
        | crate::exec::pipeline::BodyKernel::Compose { .. } => {
            if let Some(path) = kernel_to_physical_path(kernel) {
                return Some(NdjsonDirectPredicate::Path(path));
            }
            let (source_steps, element, suffix_steps) =
                direct_array_element_path_from_kernel(kernel)?;
            Some(NdjsonDirectPredicate::ArrayElementPath {
                source_steps,
                element,
                suffix_steps,
            })
...
fn direct_item_predicate_from_kernel(
    ...
    if let crate::exec::pipeline::BodyKernel::CmpLit { lhs, op, lit } = kernel {
        if op.is_scalar_comparison() {
            if let Some(lhs) = kernel_to_physical_path(lhs) {
                return Some(NdjsonDirectItemPredicate::CmpLit {
                    lhs,
                    op: *op,
                    lit: lit.clone(),
            ...
            if let Some((source_steps, element, suffix_steps)) =
                direct_array_element_path_from_kernel(lhs)
            {
                return Some(NdjsonDirectItemPredicate::ArrayElementCmpLit {
                    source_steps,
                    element,
                    suffix_steps,
                    op: *op,
    ...
    match kernel {
        ...
        crate::exec::pipeline::BodyKernel::CmpLit { lhs, op, lit } if op.is_scalar_comparison() => {
            if let Some((source_steps, element, suffix_steps, call)) =
                direct_array_element_scalar_call_from_kernel(lhs)
            {
                return Some(NdjsonDirectItemPredicate::ArrayElementViewScalarCmpLit {
                    source_steps,
                    element,
                    suffix_steps,
                    call,
        ...
        crate::exec::pipeline::BodyKernel::BuiltinCall { .. } => {
            if let Some((source_steps, element, suffix_steps, call)) =
                direct_array_element_scalar_call_from_kernel(kernel)
            {
                return Some(NdjsonDirectItemPredicate::ArrayElementViewScalarCall {
                    source_steps,
                    element,
                    suffix_steps,
                    call,
        ...
        | crate::exec::pipeline::BodyKernel::Compose { .. } => {
            let (source_steps, element, suffix_steps) =
                direct_array_element_path_from_kernel(kernel)?;
            Some(NdjsonDirectItemPredicate::ArrayElementPath {
                source_steps,
                element,
                suffix_steps,
            })
...
fn kernel_to_physical_path(
    ...
    use crate::exec::pipeline::{ArraySelector, BodyKernel};

    match kernel {
        BodyKernel::Current => Some(Vec::new()),
        BodyKernel::FieldRead(key) => Some(vec![PhysicalPathStep::Field(Arc::clone(key))]),
        BodyKernel::FieldChain(keys) => Some(physical_field_keys_to_path_steps(keys)),
        ...
        BodyKernel::Compose { first, then } => {
            let mut steps = kernel_to_physical_path(first)?;
        ...
        BodyKernel::BuiltinCall { receiver, call } => {
            ...
            steps.extend(physical_field_keys_to_path_steps(&result_keys));
            Some(steps)
        }
        _ => None,
    }
...
fn direct_projection_value_from_kernel(
    ...
    match kernel {
        ...
        crate::exec::pipeline::BodyKernel::Compose { first, then } => {
            direct_composed_projection_value_from_kernel(first, then)
        }
        _ => None,
    }
}

fn direct_composed_projection_value_from_kernel(
...
fn direct_array_element_scalar_projection_from_kernel(
    kernel: &crate::exec::pipeline::BodyKernel,
) -> Option<NdjsonDirectProjectionValue> {
    let (source_steps, element, suffix_steps, call) =
        direct_array_element_scalar_call_from_kernel(kernel)?;
    Some(NdjsonDirectProjectionValue::Nested(Box::new(
        NdjsonDirectTapePlan::ArrayElementViewScalarCall {
            source_steps,
...
fn direct_array_element_scalar_call_from_kernel(
    kernel: &crate::exec::pipeline::BodyKernel,
) -> Option<(
    NdjsonPhysicalPath,
    NdjsonDirectElement,
...
fn direct_scalar_call_from_kernel(
    ...
    if let Some(call) = kernel.path_scalar_call() {
        return Some((
...
mod tests {
    ...
    #[test]
    fn direct_tape_predicate_accepts_array_selector_scalar_calls() {
        let cmp = crate::parse::parser::parse("tags.first().len() > 2").expect("parse cmp");
        let truthy = crate::parse::parser::parse("attrs.first().value.len()").expect("parse call");

        assert!(matches!(
            direct_tape_predicate_for_expr(&cmp),
            Some(NdjsonDirectPredicate::ArrayElementViewScalarCmpLit { .. })
        ));
        assert!(matches!(
            direct_tape_predicate_for_expr(&truthy),
            Some(NdjsonDirectPredicate::ArrayElementViewScalarCall { .. })
        ));
    }
    ...
    fn direct_tape_predicate_accepts_array_selector_paths() {
        ...
        assert!(matches!(
            direct_tape_predicate_for_expr(&cmp),
            Some(NdjsonDirectPredicate::Binary { ref lhs, op: BinOp::Eq, .. })
                if matches!(
                    lhs.as_ref(),
        ...
        assert!(matches!(
            direct_tape_predicate_for_expr(&cmp_index),
            Some(NdjsonDirectPredicate::Binary { ref lhs, op: BinOp::Eq, .. })
                if matches!(
                    lhs.as_ref(),
                    NdjsonDirectPredicate::Path(steps)
        ...
        assert!(matches!(
            direct_tape_predicate_for_expr(&truthy),
            Some(NdjsonDirectPredicate::Path(ref steps))
                if matches!(
                    steps.as_slice(),
                    [
                        PhysicalPathStep::Field(attrs),
                        PhysicalPathStep::Index(0),
                        PhysicalPathStep::Field(value)
                    ] if attrs.as_ref() == "attrs" && value.as_ref() == "value"
                )
        ));
    }
