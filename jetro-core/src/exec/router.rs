...
mod tests {
    ...
    #[test]
    fn view_object_spread_projection_stays_tape_streamed() {
        let j = Jetro::from_bytes(
            br#"{"data":[{"base":{"isbn":"978","price":20},"value":"ok"},{"base":{"isbn":"979","price":30},"value":"new"}],"unused":{"large":[1,2,3,4]}}"#.to_vec(),
        )
        .unwrap();
        j.reset_tape_materialized_subtrees();

        let out = j.collect(r#"$.data.map({...base, extra: value})"#).unwrap();

        assert_eq!(
            out,
            json!([
                {"isbn": "978", "price": 20, "extra": "ok"},
                {"isbn": "979", "price": 30, "extra": "new"}
            ])
        );
        assert!(!j.root_val_is_materialized());
        assert_eq!(j.tape_materialized_subtrees(), 0);
    ...
    #[test]
    fn view_deep_object_spread_projection_keeps_root_lazy() {
        let j = Jetro::from_bytes(
            br#"{"data":[{"base":{"meta":{"tags":["sf"],"score":1},"name":"Dune"},"delta":{"meta":{"tags":["classic"],"year":1965}}}],"unused":{"large":[1,2,3,4]}}"#.to_vec(),
        )
        .unwrap();
        j.reset_tape_materialized_subtrees();
        ...
        assert_eq!(
            out,
            json!({"meta": {"tags": ["sf", "classic"], "score": 1, "year": 1965}, "name": "Dune"})
        );
        assert!(!j.root_val_is_materialized());
    }

    #[test]
    fn view_array_spread_projection_stays_tape_streamed() {
        let j = Jetro::from_bytes(
            br#"{"data":[{"head":"a","tags":["sf","hugo"],"tail":"z"},{"head":"b","tags":["new"],"tail":"y"}],"unused":{"large":[1,2,3,4]}}"#.to_vec(),
        )
        .unwrap();
        j.reset_tape_materialized_subtrees();

        let out = j.collect(r#"$.data.map([head, ...tags, tail])"#).unwrap();

        assert_eq!(out, json!([["a", "sf", "hugo", "z"], ["b", "new", "y"]]));
        assert!(!j.root_val_is_materialized());
        assert_eq!(j.tape_materialized_subtrees(), 0);
    }
    ...
    fn view_dynamic_index_projection_stays_tape_streamed() {
        ...
        assert_eq!(
            out,
            json!([
                {"picked": "beta", "numbered": 20},
                {"picked": "ex", "numbered": 3}
            ])
        );
        assert!(!j.root_val_is_materialized());
        assert_eq!(j.tape_materialized_subtrees(), 0);
    ...
    fn view_slice_projection_stays_tape_streamed() {
        ...
        assert_eq!(
            out,
            json!([
                {"prefix": [10, 20], "middle": [20, 40], "reverse": [40, 30, 20, 10]},
                {"prefix": [1, 2], "middle": [2], "reverse": [3, 2, 1]}
            ])
        );
        assert!(!j.root_val_is_materialized());
        assert_eq!(j.tape_materialized_subtrees(), 0);
