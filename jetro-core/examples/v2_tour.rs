...
fn main() {
    ...
    show("find — first match (alias of filter + [0])",
        &j.collect(r#"$.store.books.find(title == "Dune")"#).unwrap());

    show("find_all — alias of filter",
        &j.collect(r#"$.store.books.find_all(tags.includes("classic"))"#).unwrap());

    show("pick — project + rename (alias: src)",
        &j.collect("$.store.books.pick(title, slug: id, year)").unwrap());

    show("unique_by — dedup by derived key",
        &j.collect("$.store.books.sort(year).unique_by(author)").unwrap());

    show("collect — scalar → [scalar], arr → arr",
        &j.collect("$.store.books[0].tags.collect()").unwrap());

    // ── Deep search ──────────────────────────────────────────────────────────

    show("$..find — every descendant satisfying pred",
        &j.collect("$..find(@ kind number and @ < 10)").unwrap());

    show("$..shape — every object with all listed keys",
        &j.collect("$..shape({id, title, price})").unwrap());

    show("$..like — every object with listed key==lit",
        &j.collect(r#"$..like({status: "paid"})"#).unwrap());

    // ── Chain-style writes (desugar into `patch` blocks) ─────────────────────

    show(".set — replace a single leaf (returns full doc)",
        &j.collect("$.store.currency.set(\"EUR\")").unwrap());

    show(".modify — rewrite a leaf using @",
        &j.collect("$.store.books[0].price.modify(@ * 0.9)").unwrap());

    show(".unset — drop a child of the leaf object",
        &j.collect("$.store.books[0].unset(ratings)").unwrap());

    // ── Python-style ternary ─────────────────────────────────────────────────

    show("ternary — chained right-assoc",
        &j.collect(r#"
            $.store.books.map(
                {title,
    ...
    show("ternary — const-fold (else branch never compiles)",
        &j.collect("42 if true else 0/0").unwrap());

    // ── Combined: search + conditional inside projection ─────────────────────

    show("shape via pick + ternary — analyst view",
        &j.collect(r#"
            $.store.books
              .find_all(price >= 10)
