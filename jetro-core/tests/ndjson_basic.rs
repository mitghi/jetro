...
fn run_ndjson_writes_static_object_projection_directly() {
    let engine = JetroEngine::new();
    let input = br#"{"id":7,"name":"Ada","score":42,"profile":{"author":{"name":"Ada"}}}
"#;
    let mut out = Vec::new();
    ...
    engine
        .run_ndjson(
            Cursor::new(input),
            r#"{id: id, label: name.upper(), author: profile.get_path("author.name"), score: score, kind: "user"}"#,
            &mut out,
        )
    ...
    assert_eq!(
        ...
        r#"{"id":7,"label":"ADA","author":"Ada","score":42,"kind":"user"}"#.to_string() + "\n"
    );
}
...
#[test]
fn rows_stream_array_selector_path_filter_is_direct() {
    let engine = JetroEngine::new();
    let input = br#"{"id":"a","tags":[{"name":"sf"}],"attrs":[{"value":""}]}
{"id":"b","tags":[{"name":"tech"}],"attrs":[{"value":"robot"}]}
{"id":"c","tags":[{"name":"sf"}],"attrs":[{"value":"ok"}]}
"#;
    let mut out = Vec::new();
    ...
    assert_eq!(report.stats.rows_scanned, 3);
    assert_eq!(report.stats.direct_filter_rows, 5);
    assert_eq!(report.stats.fallback_filter_rows, 0);
    assert_eq!(report.stats.direct_project_rows, 1);
    assert_eq!(report.stats.fallback_project_rows, 0);
}
